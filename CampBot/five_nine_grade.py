...
def testIntent():
    ...
    inputLIST = ['何時結束','何時開始','啥時結束','啥時開始','幾點結束','幾點開始','帶小孩回家','帶小孩過去','幾點開始接','送小孩過去','什麼時候回家','什麼時候開始','把小孩送過去','甚麼時候結束','幾點可以接小孩','接送時間是幾點','甚麼時候會結束','結束時間是幾點','結束需要到幾點','可以在哪個時段接','可以在幾點之後接','可以接小孩的時間','幾點要送小孩過去','什麼時候可以接小孩','什麼時候送小孩過去','什麼時間可以接小孩','把小孩送過去的時間','營隊期間的接送時間','活動時間是幾點到幾點','需要在幾點被送到那裡','開始與結束時間是幾點到幾點','活動進行的時段是從幾點到幾點']
    testLoki(inputLIST, ['shi2jyan1'])
    print("")

    # fei4uuong4you1hwei4
    print("[TEST] fei4uuong4you1hwei4")
    inputLIST = ['團報優惠','有折扣嗎','提供優惠？','有沒有優惠','有沒有打折','可以打折嗎？','可否打個折？','是否有優待？','有優惠價格？','有優惠方案嗎','有打折優惠？','能不能給個優惠？','能給一點折扣嗎？','提供任何折扣方案？','有提供折扣或優惠？','有優惠方案可供選擇？']
    testLoki(inputLIST, ['fei4uuong4you1hwei4'])
    print("")

    # hwo2dweng4ji4lu4
    print("[TEST] hwo2dweng4ji4lu4")
    inputLIST = ['有照片','幫小孩拍照','活動的相片','相片要怎麼看','照片要去哪裡看','了解孩子參與狀況','得知孩子參與狀況','有沒有活動的相片','有活動拍攝的圖片','想要看活動照片要去哪裡找']
    testLoki(inputLIST, ['hwo2dweng4ji4lu4'])
    print("")

    # gwo4ye4
    print("[TEST] gwo4ye4")
    inputLIST = ['住哪裡','會過夜','睡哪裡','住宿的安排','晚上會住哪裡','有過夜的安排','住宿會如何安排','怎麼過去住宿地點','晚上會在哪裡住宿','過夜的營隊有哪些','過夜需要準備什麼','住宿地點會是在哪裡','住宿地點該如何前往','住宿安排會是怎樣的','住宿有哪些詳細安排','過夜要準備些什麼？','住宿的具體安排是什麼','晚上會在哪裡居住休息','晚上的住宿地點在哪裡','需要準備的東西有什麼?','到達過夜營隊的住宿地點','前往過夜營隊的住宿地點','參加哪些提供過夜的營隊','過夜營隊要準備什麼東西','所有的營隊都會在夜晚過夜','有哪些提供過夜住宿的營隊','有哪些東西是過夜需準備的','有哪些營隊可以讓孩子過夜','過夜營隊晚上所居住的地點是什麼','住宿在過夜營隊中會有什麼樣的安排','有哪些營隊可以讓孩子體驗過夜生活']
    testLoki(inputLIST, ['gwo4ye4'])
    print("")

    # hang2qyan2
    print("[TEST] hang2qyan2")
    inputLIST = ['行前通知','進入親師群','加入到親師群','被加入親師群','需要準備什麼','須要準備什麼','行前任務是什麼','需要做什麼準備','行前的工作有哪些','需做哪些準備工作','告知營隊行前的通知','應該做哪些準備事項','提供營隊行前的通知','有沒有營隊行前的通知','應該先作哪些準備的安排','行前應該要做什麼準備工作','行前須要完成的事情是什麼','營隊行前要完成的任務有哪些']
    ...
    inputLIST = ['推薦營隊','參加什麼營','想了解營隊','有哪些營隊','營隊在幹嘛','營隊有哪些','營隊在做什麼','適合什麼營隊','推薦適合的營隊','有適合的營隊嗎','參加何種營隊活動？','有什麼營隊可參加？','有哪些營隊能夠報名？','要報名參加哪個營隊？','可以參加的營隊有哪些？','有哪幾個營隊可以參與？','有多少種營隊可以選擇？','考慮參加什麼樣的營隊？','有哪些營隊是可以報名的？','有多少種營隊可以選擇報名？']
    testLoki(inputLIST, ['ying2dwei4lei4xying2'])
    print("")

    # bao4mying2
    print("[TEST] bao4mying2")
    inputLIST = ['參加','報名','要參加','要報名','去你們營隊','想報名參加','有意願參加','要怎麼報名','想幫小孩報名','想讓小孩參加','報名方式是什麼','如何報名參加？','如何進行報名？','希望給小孩報名','想要幫小孩報名','打算幫小孩報名','有意幫小孩報名','讓我家小孩參加','去哪裡可以報名？','幫姐姐的姊姊報名','幫親人的親人報名','送小孩去你們營隊','幫我姊姊的親人報名','幫我姐姐的姊姊報名','幫我親人的姊姊報名','幫我親人的小朋友報名']
    testLoki(inputLIST, ['bao4mying2'])
    print("")

    # fen1zu3
    print("[TEST] fen1zu3")
    inputLIST = ['分在一組','怎麼分組','分在不同組','是老師分組嗎','老師會分組嗎','與好友同組。','誰會負責分組','跟朋友同一組','如何安排分組？','如何進行分組？','怎麼劃分小組？','分組方法是怎樣的？','分組的方式是什麼？','是老師要進行分組嗎？','是由老師來進行分組是嗎？']
    testLoki(inputLIST, ['fen1zu3'])
    print("")

    # ying2dwei4shou1hwo4
    print("[TEST] ying2dwei4shou1hwo4")
    inputLIST = ['有收穫','學到什麼','得到什麼','有收穫嗎','有沒有收穫','能學到什麼','培養什麼實力','學到些什麼？','獲得什麼經驗','累積什麼經驗','能獲得哪些知識？','可以獲得怎樣的知識','可以從中學到哪些東西？','可以從中獲得哪些收穫？']
    ...
    inputLIST = ['同年齡？','是相同年紀','分組都是混齡','包含不同年齡層','以不同年齡混合分組','年紀小會不會跟不上','同組的小孩都會是同年紀嗎?','同組的孩子是否都是同年齡？','年紀小的孩子會不會玩不起來?','年紀比較大的孩子會不會無聊?','同一組的小孩是否都是同年齡？','年齡較大的孩子是否會感到無聊','較大年紀的孩子是否會感到無聊','年紀小的小孩能不能融入這個活動','年紀比較大的孩子會不會覺得無聊','年紀比較大的小孩可能會感到無趣','年紀比較大的小孩會不會感到無聊','年紀小的孩子是否能跟得上活動的節奏','在分組時不同年齡的人是不是都混合在一起']
    ...
    inputLIST = ['保保險','有保險','負責保險','購買保險','有包含保險','有提供保險','是否提供保險','有投什麼保險','有沒有投保險','投保了什麼保險','需要自己保保險','是否要自己負責保險','是否需要自己投保保險','是否需要自行購買保險','在保險方面做了哪些準備？','在保險方面有怎樣的安排？']
    testLoki(inputLIST, ['bao3xyan3'])
    print("")

    # fei4uuong4twei4fei4
    print("[TEST] fei4uuong4twei4fei4")
    inputLIST = ['退費','辦退費','可以退費','想要退費','辦理退款','退費怎麼辦理','可不可以退費？','是否可以退費？','可以申請退費嗎？','可以要求退費嗎？','會如何辦理退費呢','退費要如何進行？','退費要怎麼辦理？']
    testLoki(inputLIST, ['fei4uuong4twei4fei4'])
    print("")

    # gweng4can1
    print("[TEST] gweng4can1")
    inputLIST = ['會餓','吃不飽','帶零食','有點心','肚子餓','訂午餐','供應午餐','自備午餐','午餐吃什麼','帶零食進去','會餓怎麼辦','有沒有點心','中午會吃什麼','把零食帶進去','自行攜帶午餐','有沒有點心吃？']
    ...
    inputLIST = ['吃素','吃藥','不能吃','有急事','有其他問題','臨時有事情','暫時有緊急事項','有其他問題怎麼辦','突發情況需要找誰','臨時有狀況要找誰']
