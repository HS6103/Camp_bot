...
try:
    ...
    from intentFIVENINE import Loki_Ending
...
try:
    ...
    USERNAME = accountInfo["username_59"]
        ...
        for index, key in enumerate(inputLIST):
            ...
            for resultIndex in range(0, lokiRst.getLokiLen(index)):
                ...
                if lokiRst.getIntent(index, resultIndex) == "Ending":
                    lokiResultDICT = Loki_Ending.getResult(key, lokiRst.getUtterance(index, resultIndex), lokiRst.getArgs(index, resultIndex), lokiResultDICT, refDICT)
...
def testIntent():
    # di4dyan3
    print("[TEST] di4dyan3")
    inputLIST = ['地點在哪裡?','地點位於哪裡？','地點都在台北嗎?','請問地點在哪邊？','什麼時候會知道地點?','請問地點在哪個位置？','何時才能得知具體位置？','全部的地點都在臺北嗎？','地點的具體位置是哪裡？','要到什麼時候才能確定地點？','要到何時才能知道確切位置？','非台北地區有其他活動地點嗎?','什麼時候才能取得地點的資訊？','除了台北之外，還有其他場地嗎?','甚麼時候才會得知地點的具體情況？','除了臺北外，還有別的地點可供選擇嗎?','臺北以外有沒有其他可以參加營隊的地方？','除了臺北以外，還有其他的地方可以考慮嗎?','非臺北地區還有沒有別的營隊地點可供選擇？']
    testLoki(inputLIST, ['di4dyan3'])
    print("")

    # hwen4lying2
    print("[TEST] hwen4lying2")
    inputLIST = ['分組都是混齡嗎?','同組的小孩都會是同年紀嗎?','同組的孩子是否都是同年齡？','年紀小的孩子會不會玩不起來?','年紀比較大的孩子會不會無聊?','同一組的小孩是否都是同年齡？','營隊是否以不同年齡混合分組？','較大年紀的孩子是否會感到無聊？','年紀小的小孩能不能融入這個活動？','年齡較大的孩子是否會感到無聊呢？','是不是每個分組都包含不同年齡層？','年紀比較大的孩子會不會覺得無聊呢？','年紀比較大的小孩可能會感到無趣嗎？','年紀比較大的小孩會不會感到無聊呢？','年紀小的孩子是否能跟得上活動的節奏？','在分組時不同年齡的人是不是都混合在一起？']
    ...
    inputLIST = ['有折扣嗎','提供優惠？','有沒有優惠','有沒有打折','可以打折嗎？','可否打個折？','是否有優待？','有優惠價格？','有優惠方案嗎','有打折優惠？','能不能給個優惠？','能給一點折扣嗎？','提供任何折扣方案？','有提供折扣或優惠？','有優惠方案可供選擇？']
    testLoki(inputLIST, ['fei4uuong4you1hwei4'])
    print("")

    # hwo2dweng4ji4lu4
    print("[TEST] hwo2dweng4ji4lu4")
    inputLIST = ['拍照','有照片','活動的相片','相片要怎麼看','照片要去哪裡看','了解孩子參與狀況','得知孩子參與狀況','有沒有活動的相片','有活動拍攝的圖片','想要看活動照片要去哪裡找']
    testLoki(inputLIST, ['hwo2dweng4ji4lu4'])
    print("")

    # gwo4ye4
    print("[TEST] gwo4ye4")
    inputLIST = ['會過夜','有過夜的安排','過夜的營隊有哪些','低年級有過夜的營隊','住宿有哪些詳細安排','住宿的具體安排是什麼','晚上會在哪裡居住休息','晚上的住宿地點在哪裡','過夜營隊晚上會住哪裡','低年級有安排過夜的營隊','到達過夜營隊的住宿地點','前往過夜營隊的住宿地點','參加哪些提供過夜的營隊','得知過夜營隊住宿的安排','過夜營隊要準備什麼東西','告訴我過夜營隊住宿的細節','所有的營隊都會在夜晚過夜','有哪些提供過夜住宿的營隊','有哪些東西是過夜需準備的','有哪些營隊可以讓孩子過夜','過夜營隊的住宿會如何安排','過夜需要準備的東西有什麼?','低年級有提供過夜活動的營隊','在過夜營隊晚上會在哪裡住宿','過夜營隊要怎麼過去住宿地點','過夜營隊的住宿地點該如何前往','過夜營隊的住宿安排會是怎樣的','過夜營隊晚上所居住的地點是什麼','住宿在過夜營隊中會有什麼樣的安排','有哪些營隊可以讓孩子體驗過夜生活','過夜營隊晚上的住宿地點會是在哪裡']
    testLoki(inputLIST, ['gwo4ye4'])
    print("")

    # hang2qyan2
    print("[TEST] hang2qyan2")
    inputLIST = ['進入親師群','加入到親師群','被加入親師群','有營隊行前通知','提供營隊行前通知','被邀請加入親師群','何時會被納入親師群','告知營隊行前的通知','提供營隊行前的通知','營隊行前任務是什麼','有沒有營隊行前的通知','營隊行前的工作有哪些','營隊行前需要做什麼準備','營隊開始前需要準備什麼','進入營隊前要做哪些準備','進入營隊前須要準備什麼','營隊行前需做哪些準備工作','營隊行前要完成的任務有哪些','參加營隊前應該做哪些準備事項','營隊行前應該要做什麼準備工作','營隊行前須要完成的事情是什麼','進營隊之前應該先作哪些準備的安排']
    testLoki(inputLIST, ['hang2qyan2'])
    print("")

    # ying2dwei4lei4xying2
    print("[TEST] ying2dwei4lei4xying2")
    inputLIST = ['推薦營隊','參加什麼營','想了解營隊','有哪些營隊','適合什麼營隊','推薦適合的營隊','有適合的營隊嗎','參加何種營隊活動？','有什麼營隊可參加？','有哪些營隊能夠報名？','要報名參加哪個營隊？','可以參加的營隊有哪些？','有哪幾個營隊可以參與？','有多少種營隊可以選擇？','考慮參加什麼樣的營隊？','有哪些營隊是可以報名的？','有多少種營隊可以選擇報名？']
    testLoki(inputLIST, ['ying2dwei4lei4xying2'])
    print("")

    # an1quuan2cwo4shi1
    print("[TEST] an1quuan2cwo4shi1")
    inputLIST = ['不舒服','會受傷','有危險','發生危險','碰上危險','確保安全','受傷怎麼辦','有安全措施','有防護措施','身體有狀況','如何處理受傷','出現危險的情況','受傷了該怎麼辦','有什麼安全措施','有安全保護措施','有沒有安全措施','有相關的安全措施']
    testLoki(inputLIST, ['an1quuan2cwo4shi1'])
    print("")

    # te4shu1xuu1qyou2
    print("[TEST] te4shu1xuu1qyou2")
    inputLIST = ['吃素','吃藥','有其他問題','有急事找誰','有特殊需求','臨時有事情','暫時有緊急事項','有其他問題怎麼辦','突發情況需要找誰','臨時有狀況要找誰']
    testLoki(inputLIST, ['te4shu1xuu1qyou2'])
    print("")

    # bao3xyan3
    print("[TEST] bao3xyan3")
    inputLIST = ['有包含保險','有提供保險','是否提供保險','有投什麼保險','有沒有投保險','投保了什麼保險','需要自己保保險','是否要自己負責保險','是否需要自己投保保險','是否需要自行購買保險','在保險方面做了哪些準備？','在保險方面有怎樣的安排？']
    testLoki(inputLIST, ['bao3xyan3'])
    print("")

    # fei4uuong4twei4fei4
    print("[TEST] fei4uuong4twei4fei4")
    inputLIST = ['可以退費','想要退費','辦理退款','退費怎麼辦理','可不可以退費？','是否可以退費？','可以申請退費嗎？','可以要求退費嗎？','會如何辦理退費呢','退費要如何進行？','退費要怎麼辦理？']
    testLoki(inputLIST, ['fei4uuong4twei4fei4'])
    print("")

    # fen1zu3
    print("[TEST] fen1zu3")
    inputLIST = ['分在一組','怎麼分組','分在不同組','是老師分組嗎','老師會分組嗎','與好友同組。','跟朋友同一組','如何安排分組？','如何進行分組？','怎麼劃分小組？','分組方法是怎樣的？','分組的方式是什麼？','是老師要進行分組嗎？','是由老師來進行分組是嗎？']
    testLoki(inputLIST, ['fen1zu3'])
    print("")

    # gweng4can1
    print("[TEST] gweng4can1")
    inputLIST = ['帶零食','有點心','供應午餐','自備午餐','午餐吃什麼','帶零食進去','有沒有點心','中午會吃什麼','把零食帶進去','自行攜帶午餐','有沒有點心吃？']
    testLoki(inputLIST, ['gweng4can1'])
    print("")

    # pyin3zhi2
    print("[TEST] pyin3zhi2")
    inputLIST = ['帶幾個小孩','師生比是多少','確保教師素質','如何把關教學品質','如何確保教學品質','師生比例是多少？','確保營隊師資品質','老師和學生的比例','如何控制教學品質？','如何管理教學品質？','如何維護教學品質？','師生之間的比例是多少？','老師和學生的比例是多少？','確保教師具備高水準的專業能力']
    testLoki(inputLIST, ['pyin3zhi2'])
    print("")

    # ying2dwei4shou1hwo4
    print("[TEST] ying2dwei4shou1hwo4")
    inputLIST = ['有收穫','學到什麼','得到什麼','有收穫嗎','有沒有收穫','獲得的東西','能學到什麼','培養什麼實力','獲得什麼經驗','累積什麼經驗','能夠學到些什麼？','能獲得哪些知識？','可以獲得怎樣的知識','可以從中學到哪些東西？','可以從中獲得哪些收穫？']
    testLoki(inputLIST, ['ying2dwei4shou1hwo4'])
    print("")

    # bao4mying2
    print("[TEST] bao4mying2")
    inputLIST = ['要參加','要報名','想報名參加','有意願參加','要怎麼報名','想幫小孩報名','想讓小孩參加','報名方式是什麼','如何報名參加？','如何進行報名？','希望給小孩報名','想要幫小孩報名','打算幫小孩報名','有意幫小孩報名','讓我家小孩參加','去哪裡可以報名？','幫姐姐的姊姊報名','幫親人的親人報名','幫我姊姊的親人報名','幫我姐姐的姊姊報名','幫我親人的姊姊報名','幫我親人的小朋友報名']
